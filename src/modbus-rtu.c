    ...
    modbus_t* ctx, int function, int addr, int nb, uint8_t* req)
{
    ...
    if ((MODBUS_FC_READ_COILS == function) || 
        ...
        (MODBUS_FC_WRITE_SINGLE_COIL == function) ||
        (MODBUS_FC_WRITE_SINGLE_REGISTER == function) ||
        (MODBUS_FC_READ_EXCEPTION_STATUS == function) ||
        (MODBUS_FC_WRITE_MULTIPLE_COILS == function) ||
        (MODBUS_FC_WRITE_MULTIPLE_REGISTERS == function) ||
        (MODBUS_FC_REPORT_SLAVE_ID == function) ||
        (MODBUS_FC_MASK_WRITE_REGISTER == function) ||
        (MODBUS_FC_WRITE_AND_READ_REGISTERS == function))
    {
        req[2] = addr >> 8;
        req[3] = addr & 0x00ff;
        req[4] = nb >> 8;
