...
static int user_rdwt_register(modbus_t *ctx)
{
    ...
    req_length = ctx->backend->build_request_basis(ctx, gMb_Packet.fc, 0, 0, req);

    for(int i = 0; i < gMb_Packet.tx_pdu_length; i++)
    {
        req[i + 2] = gMb_Packet.tx_frame[i];
    }
